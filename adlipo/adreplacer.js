  ...
  adreplacer = {
    ...
    _doReplace: function (el) {
      ...
      if (w && h && w > 50 && h > 40) {
        ...
        div.setAttribute('style', 'width: ' + w + 'px; height:' + h + 'px; ' + 'background-color: ' + this.palette[++this.count % this.palette.length] + '; ' + 'color: #ffffff; font-family: custom; fontSize: 20px; text-align: center; ' + 'display: table-cell; vertical-align: middle; ' + 'float: ' + this._getFloatStyle(el)); //+'; overflow: hidden;'); // fixes nytimes top-banner
    ...
    _remoteReplace: function (w, h, pad) {
      ...
      req.onload = function () {
        ...
        var poem = JSON.parse(req.responseText),
          html = '';
      ...
      newPic.addEventListener("mouseover", function (e) {
        ...
        if (typeof jQuery !== "undefined") {
          ...
          chrome.extension.sendRequest({
              command: "picreplacement_inject_jquery",
              allFrames: (window !== window.top)
            },
            after_jquery_is_available
          );
        }
