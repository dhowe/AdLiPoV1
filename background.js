  ...
  if (!SAFARI) {
    ...
    function onBeforeRequestHandler(details) {
      ...
      if (canPurge && blocked) {
		...
		log("background.js: chrome.tabs.sendRequest:", data);
      ...
      log("Block result", blocked, details.type, frameDomain, details.url.substring(0, 100));
  ...
  if (!SAFARI) {
    ...
    updateButtonUIAndContextMenus = function() {
      ...
      function setBrowserButton(info) {
        if (adblock_is_paused()) {
          chrome.browserAction.setIcon({path:{'19': "adlipo/img/icon19-grayscale.png", '38': "img/icon38-grayscale.png"}, tabId: info.tab.id});
          ...
          chrome.browserAction.setIcon({path:{'19': "adlipo/img/icon19-whitelisted.png", '38': "img/icon38-whitelisted.png"}, tabId: info.tab.id});
        } else {
          chrome.browserAction.setIcon({path:{'19': "adlipo/img/icon19.png", '38': "img/icon38.png"}, tabId: info.tab.id});
        }
      }
  ...
  (function() {
	...
	console.log("background.js: chrome.extension.onRequest.addListener('sendResponse');");
